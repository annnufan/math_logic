		...
		if (v->type == 0 && *(v->second) == *(x)) {
			for (int j = 0; j < (int)evidence.size()  -1; j++) {
				if (*(evidence[j]) == *(v->first))  {
					d[(int)evidence.size()] = make_pair(j+1, i+1); 
					// fout << "(" << (int)evidence.size() << ") MP\n" << j << ' ' << i << "\n"; 
					return;
				}
			}
		}
		// if  (x->type == 0 && *(x->second) == *(v)) {
		// 	fout << "(" << (int)evidence.size() << ") WARNING\n" << i << "\n"; 
		// }
	}
...
//@
	if ((x->type == 0) && (x->second->type == 5)) {
		for (int i = 0; i < (int)evidence.size() - 1; i++) {
			expression *v = evidence[i];
			expression change;
			if (v->type == 0 && *(v->first) == *(x->first) && *(v->second) == *(x->second->second) && !consist_free(x, error, 3)) {
				d[(int)evidence.size()] = make_pair(i+1, 0); 
				return;
			}
		}
	}
...
//?
	if ((x->type == 0) && (x->first->type == 6)) {
		for (int i = 0; i < (int)evidence.size() - 1; i++) {
			expression *v = evidence[i];
			expression change;
			if (v->type == 0 && *(v->second) == *(x->second) && *(v->first) == *(x->first->second) && !consist_free(x, error, 4)) {
				d[(int)evidence.size()] = make_pair(i+1, 0); 
				return;
			}
		}
	}  
	// fout << "I don't know what it is\n";
}
